...
Я начал с сканирования `nmap`. Если закончится идея, я могу запустить autorecon, но со временем мне всё меньше нравится, и я чувствую, что усваиваю больше от ручного перечисления.

Первоначальное сканирование nmap:
...
Ключевые выводы:
- HTTP-сервер на порту 80, запускает HTTPFileServer 2.3
- Компьютер на Windows, версию неизвестна

Вот и всё.

### Сканирование всех портов

Я также выполнил быстрое сканирование всех портов.
...
Это не выявило новых портов.

### Перечисление ОС

Я попробовал дальше перечислить ОС с флагом `-O`.
...
Наиболее вероятная система — Windows Server 2012.

## Gobuster

Я выполнил быстрое сканирование корневой папки сайта:
