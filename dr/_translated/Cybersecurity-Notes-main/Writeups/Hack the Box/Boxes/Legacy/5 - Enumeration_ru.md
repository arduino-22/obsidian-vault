...
Я начал с стандартного сканирования `nmap`, используя `-Pn` для отключения пинговых проверок:
...
Это показывает, что имя компьютера — `LEGACY`, а операционная система — Windows XP. Единственные открытые порты — 139 для `netbios` и 445 для `SMB`.

Пока это сканирование происходило, в другом окне я начал полное сканирование портов с `sleep 300`, чтобы оно началось через пять минут:
...
Не было новых открытых портов.

Поскольку это старая операционная система и порт 445 открыт, я выполнил сканирование `vuln` nmap, чтобы проверить, уязвим ли система для [[CVE-2017-0143|Eternal Blue]]:
...
|     Risk factor: HIGH
|       A critical remote code execution vulnerability was found.
|       Предупреждение: Уязвимость может быть использована для атаки.
